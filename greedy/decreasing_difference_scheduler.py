#!/usr/bin/env python
"""
Schedule jobs in decreasing order of the difference (weight - length) and report the sum of weighted completion times of
...
def merge_schedule(list1, list2):
    ...
    while i < left_len and j < len(list2):
        left_difference = int(list1[i][0]) - int(list1[i][1])
        right_difference = int(list2[j][0]) - int(list2[j][1])

        if left_difference > right_difference:
            merged.append(list1[i])
            i += 1
        elif right_difference > left_difference:
